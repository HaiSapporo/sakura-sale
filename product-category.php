    ...
    if( ($_REQUEST['type'] != 'top-category') && ($_REQUEST['type'] != 'mid-category') && ($_REQUEST['type'] != 'end-category') ) {
        ...
        if($_REQUEST['type'] == 'top-category') {
            if(!in_array($_REQUEST['id'],$top)) {
                ...
                $final_ecat_ids = array_merge($arr0,$arr1,$arr2);
    ...
    <div class="container">
        <div class="row">
          <div class="col-md-3">
            ...
            <div class="col-md-9">
                <div class="alert alert-info alert-dismissible fade show" style="tex-align:center;">
    		<strong>Info!</strong>
   			 - Tất cả các sản phẩm được sắp xếp tăng dần theo giảm giá, càng về cuối giá càng giảm mạnh.
                ...
                <div class="product product-cat">
                    ...
                    <div class="row">
                        ...
                        if($prod_count==0) {
                            ...
                            for($ii=0;$ii<count($final_ecat_ids);$ii++) {
                                $statement = $pdo->prepare("SELECT * FROM tbl_product WHERE discounted = 'true' AND ecat_id=? AND p_is_active=? ORDER BY import_date desc");
                                ...
                                foreach ($result as $row) {
                                    ...
                                    <div class="col-md-4 item item-product-cat">
                                        <div class="inner">
                                            ...
                                            <div class="text">
                                            <h3><a href="<?php echo $row['url']; ?>"  target="_blank"><?php echo mb_strimwidth($row['title'], 10, 40); ?></a></h3>
                                                <h4>
                                                    <?php echo LANG_VALUE_1; ?><?php echo format_yen_for_display($row['p_current_price']); ?> 
                                                ...
                                                <?php else: ?>
                                                    <p><a href="<?php echo $row['url'];?>" target="_blank">Giảm giá <?php echo $row['savings_percent'];?>%</a></p>
                                                <?php endif; ?>
                                                <div class="">
                                                    Cập nhật lúc: <?php echo $row['import_date']; ?>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
